  ...
  */
  ...
  var PageSlider = PageControl.subclass({
    ...
    setPage: function(){
      ...
      var pageWidth = currentPage.element.offsetHeight;
      var pagePosition = currentPage.element.offsetTop;
      var pageScrollTo;

      if (this.scroller.currentVelocityY)
      {
        pageScrollTo = this.scroller.currentVelocityY > 0
          ? currentPage.nextSibling
          : currentPage.previousSibling;
      }
      else
        if ((this.scroller.viewportY > (pagePosition + pageWidth / 2))
            || (this.scroller.viewportY < (pagePosition - pageWidth / 2)))
        {
          pageScrollTo = this.scroller.viewportY - pagePosition > 0
            ? currentPage.nextSibling
            : currentPage.previousSibling;
        }
    ...
    adjustRotation: function(leftToRight){
      ...
      var childWidth = this.firstChild.element.offsetHeight;
